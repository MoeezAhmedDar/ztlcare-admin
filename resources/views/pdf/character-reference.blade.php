...
<head>
    ...
    <style>
        ...
        /* Waves background - same as other letters */
        .waves {
            position: fixed;
            bottom: 0;
            left: 50%;
        ...
        /* Date and Reference on the EXACT same line */
        .date-reference-line {
            position: relative;
            margin: 20px 0 12px 0;  /* Reduced bottom margin for normal spacing */
            line-height: 1.4;
            font-size: 10.4pt;
        }
        ...
        /* Remove default top margin from first paragraph after date */
...
<body>
    ...
    <div class="content">
        ...
        <!-- DATE AND REFERENCE - PERFECTLY ALIGNED ON SAME LINE -->
        <div class="date-reference-line">
            ...
            <div class="reference-part">
                <strong>Reference: ZTL-CC-00{{ $letter->id ?? '____' }}</strong>
            </div>
        </div>

        <!-- MAIN LETTER CONTENT - NORMAL SPACING AFTER DATE -->
        <div class="letter-body" style="text-align:justify;">
            <p>Dear {{ $letter->dear ?? '' }},</p>
            
            <p>Re: Reference Request for {{ $letter->applicant->full_name ?? '' }}</p>
            <p>The above has applied for the post of {{ $letter->position ?? '' }} at ZTL Care and has named you as a character referee.</p>
            
            <!-- MAIN LETTER CONTENT - DYNAMIC -->
<div style="text-align:justify; margin-top:20px;">
    @if(!empty(trim($letter->custom_body ?? '')))
        {!! nl2br(e($letter->custom_body)) !!}
    @else
        <!-- DEFAULT TEXT -->        
